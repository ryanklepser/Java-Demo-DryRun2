...
public abstract class BaseReverseCronologicalBatchPidReader implements ItemReader<List<Long>>, ItemStream {
	...
	public static JobParameters buildJobParameters(String theOperationName, Integer theBatchSize, RequestListJson theRequestListJson) {
		...
		map.put(MultiUrlJobParameterValidator.JOB_PARAM_OPERATION_NAME, new JobParameter<>(theOperationName, String.class));
		map.put(BatchConstants.JOB_PARAM_REQUEST_LIST, new JobParameter<>(theRequestListJson.toJson(), String.class));
		map.put(BatchConstants.JOB_PARAM_START_TIME, new JobParameter<>(DateUtils.addMinutes(new Date(), CommonBatchJobConfig.MINUTES_IN_FUTURE_TO_PROCESS_FROM), Date.class));
		if (theBatchSize != null) {
			map.put(BatchConstants.JOB_PARAM_BATCH_SIZE, new JobParameter<>(theBatchSize.longValue(), Long.class));
		}
